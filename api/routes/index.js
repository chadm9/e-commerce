...
router.get('/productlines/:productLines/get', (req, res)=>{
    // res.json({msg:"test"})
    var pl = req.params.productLines;
        ...
        if(results[0] !==undefined){
            connection.query(getCartTotals,[results[0].id],(error3,results3)=>{
                if(error3){
                    ...
                    var getCartContents = `SELECT * FROM cart
    ...
    connection.query(checkLoginQuery, [email], (error,results)=>{
        ...
        if(results.length === 0){
            ...
            if(checkHash){
                ...
                var updateToken = `Update users SET token=?, tokenexp=DATE_ADD(NOW(), INTERVAL 1 HOUR) WHERE email=?`
                var token = randToken.uid(40);
